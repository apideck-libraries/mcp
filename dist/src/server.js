...
export const createServer = (opts) => {
    ...
    const server = new McpServer({ name: 'apideck-mcp-native', version }, { capabilities: { tools: {} } });
    ...
    const compose = (tool) => wrapIfAnalytics(wrapWithContext(tool));
    const registerMetaTool = (name, description, inputSchema, handler) => {
        const synthetic = {
            name,
            description,
            domain: 'meta',
        ...
        server.registerTool(name, { description, inputSchema }, compose(synthetic).handler);
    };
    if (opts.mode === 'code') {
        ...
        registerTool(server, compose(createRunTool(allTools)), {});
        return server;
    }
